...
module Consul
  module Cookbook
    module Helpers
      ...
      def extract_path
        platform_family?('windows') ? node.config_prefix_path : '/opt/consul'
      end
      ...
      def consul_program(extract_path)
        @program ||= join_path(extract_path, new_resource.version, 'consul')
        windows? ? @program + '.exe' : @program
      end

      def binary_basename(version)
        case node['kernel']['machine']
          when 'x86_64', 'amd64' then ['consul', version, node['os'], 'amd64'].join('_')
          when /i\d86/ then ['consul', version, node['os'], '386'].join('_')
          when /^arm/ then ['consul', version, node['os'], 'arm'].join('_')
          else ['consul', version, node['os'], node['kernel']['machine']].join('_')
        end.concat('.zip')
      end

      def binary_checksum(version)

    ...
    class ConsulInstallationBinary < Chef::Provider
      ...
      def self.default_inversion_options(node, resource)
        ...
        super.merge(extract_to: extract_path,
                    ...
                    archive_url_root: node['consul']['archive_url_root'],
                    archive_url: '%{archive_url_root}/consul/%{version}/%{basename}',
      ...
      def self.binary_basename(node, resource)
        filename = node['consul']['enterprise'] ? 'consul-enterprise' : 'consul'
      ...
      def self.binary_checksum(node, resource)
        ...
        if node['consul']['enterprise'] then  # please note these shasums probably won't work for everyone
          case [node['os'], tag].join('-')
          when 'darwin-amd64'
            case resource.version
            when '1.4.2' then 'fd85a162c42aca9b9a62f81e5325a204475e9b2e6b5ae086e2f251b9ec5d013b'
            when '1.6.1' then 'c213ce8116c76ce9e75f29e7b55a247f3541d5086be965e11056546a82eb8964'
            end
          when 'darwin-i386'
            case resource.version
            when '1.4.2' then 'ab56f911b2d451ad0430427546325e6859011b942fb3e6682e933150aad07dfa'
            when '1.6.1' then '9b51f1c0a1bccd53afeb9f51884c48b62b9f41adc8ecf6340094ed21abd43faf'
            end
          when 'solaris-amd64'
            case resource.version
            when '1.4.2' then 'bb807523250a5122c45e99ab57b6003f2c6f029752db9d23a4cb5c4424adb32e'
            when '1.6.1' then '8c93cdea3797b33796a12975045341af6e8d9ee3886c3e0d01f5e6c2504fc685'
            end
          when 'windows-amd64'
            case resource.version
            when '1.4.2' then '9a2c3852234737f32f24e59fc0247eb219bf88a6e05c8e635c448fd1f4939f4f'
            when '1.6.1' then '8862298e4e598eb327fb30dd6a4719ccea107727bb83c327492d7841419ece18'
            end
          when 'windows-i386'
            case resource.version
            when '1.4.2' then 'c014bc914e471288f2a88bc190e10876b3477461fa02f198e336248204904c38'
            when '1.6.1' then '3e2f853a64270662ceaafec06c8f231e93abddc438dd08d5c286ebe7f6417850'
            end
          when 'linux-amd64'
            case resource.version
            when '1.4.2' then '3ee4cd693f90eab7621bf748154d736a8e83c39c7122bb0ad2fd40880316ec60'
            when '1.6.1' then '2bcb951dcb6f09fb3af20c81ff1efa63561580617f78c2b5c01c670cae8821f9'
            end
          when 'linux-i386'
            case resource.version
            when '1.4.2' then 'b9e99b5ef50663824e193f3255472c428192ab24cb467897b964926f82745bfd'
            when '1.6.1' then 'de65598efb5c8982f448b3e6297269734f0d2beb4442ac33d34034669ff932ea'
            end
          when 'linux-arm'
            case resource.version
            when '1.4.2' then '4917dfacda8234196232f3bc4d90f6605c72d45c1c159b193423f6f8917420e4'
            when '1.6.1' then '06b462ad941c1ab4b1c2569f63351e5d7045f9614023fb3b1fd00a6053b90099'
            end
          end
        else
          case [node['os'], tag].join('-')
